...
def train_vanilla_classier(
    ...
    for epoch in range(epochs):
        ...
        for batch_i,batch in enumerate(train_dataloader):
            ...
            loss = loss_function_train (one_hot_predictions, one_hot_labels.requires_grad_(True))
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for batch in validation_dataloader:
                ...
                loss = loss_function_validation (one_hot_predictions, one_hot_labels.requires_grad_(True))

                validation_loss        += loss
                validation_data_count  += one_hot_labels.size(0)
                validation_corrects    += correct_predictions_d
                validation_predictions_f1.extend(one_hot_predictions.detach().cpu().max(1)[1])
                validation_true_labels_f1.extend(one_hot_labels.detach().cpu().max(1)[1])
            ...
            validation_f1                = f1_score(validation_true_labels_f1_np, validation_predictions_f1_np, average='micro')
